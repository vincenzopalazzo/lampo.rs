...
impl InProcessProtocolHandler {
    ...
    pub fn new(network: Network, seed: &[u8; 32]) -> Self {
        ...
        let builder =
            HandlerBuilder::new(network, 0, services, seed.to_owned()).allowlist(allowlist);
        ...
        init_handler
            .handle(msgs::Message::HsmdInit2(init))
            .expect("HSMD init failed");
...
impl Transport for GrpcProtocolHandler {
    fn node_call(&self, message: Vec<u8>) -> ClientResult<Vec<u8>> {
        tokio::task::block_in_place(|| {
            self.async_runtime
                .block_on(GrpcProtocolHandler::do_call_async(
                    self.sender.clone(),
                    message,
                    None,
                ))
