...
pub struct LampoTesting {
    ...
    /// Event-processor task. `restart` must await this after shutdown so the
    /// old persistence lock is gone before the replacement opens the store.
    daemon_task: Option<tokio::task::JoinHandle<Result<(), lampo_common::ldk::io::Error>>>,
...
impl LampoTesting {
    ...
    async fn new_inner(
        ...
        lampo_conf.core_user = values.as_ref().map(|v| v.user.trim().to_owned());
        lampo_conf.core_pass = values.map(|v| v.password.trim().to_owned());
        ...
        let rpc_url = format!("http://{}:{}", lampo_conf.api_host, lampo_conf.api_port);
        log::info!("httpd client url `{rpc_url}`");
        let handler = Arc::new(HttpdHandler::new(rpc_url)?);
        ...
        let daemon_task = lampo.clone().listen();
    ...
    pub async fn restart(mut self) -> error::Result<Self> {
        ...
        let daemon_task = self.daemon_task.take();
        ...
        // replacement store against the same destination. The event processor
        // is awaited first so its final persist finishes and releases the store.
        drop(self);
        if let Some(task) = daemon_task {
            match tokio::time::timeout(Duration::from_secs(15), task).await {
                Ok(Ok(Ok(()))) => {}
                Ok(Ok(Err(err))) => {
                    log::warn!("daemon task ended with `{err}` during restart");
                }
                Ok(Err(err)) => {
                    log::warn!("daemon task panicked during restart: {err}");
                }
                Err(_) => {
                    log::warn!("daemon task did not exit within 15s of shutdown");
                }
            }
        }
    ...
    async fn mine(&self, blocks: u64) -> error::Result<()> {
        ...
        let addr = lampo_common::bitcoin::Address::from_str(&addr.address)
            .unwrap()
            .assume_checked();
        // bitcoind's cookie RPC client intermittently returns curl error 43
        // (`CURLE_BAD_FUNCTION_ARGUMENT`) under parallel tests. The call is
        // idempotent from the test's point of view: retry it instead of
        // failing the suite on the first transient argument error.
        let mut last_err = None;
        for attempt in 1..=5 {
            match self.btc.client.generate_to_address(blocks as usize, &addr) {
                Ok(mined) => {
                    last_err = None;
                    let _ = mined;
                    break;
                ...
                Err(err) => {
                    log::warn!("generate_to_address failed (attempt {attempt}/5): {err}");
                    last_err = Some(err);
        ...
        if let Some(err) = last_err {
            error::bail!("generate_to_address failed after retries: {err}");
        }
    ...
    pub async fn fund_wallet(&self, blocks: u64) -> error::Result<()> {
        ...
        async_wait!(async {
            ...
            // A transient bitcoind curl error surfaces here as an RPC failure.
            // The wait already retries; do not panic the test on one bad call.
            let funds: response::Utxos = match self.inner.call("funds", json::json!({})).await {
                Ok(funds) => funds,
                Err(err) => {
                    log::warn!("funds RPC failed while waiting for coins: {err}");
                    return Err(());
                }
            };
            ...
            let tip = match wallet.wallet_tips().await {
                Ok(tip) => tip,
                Err(err) => {
                    log::warn!("wallet tip failed while waiting for coins: {err}");
                    return Err(());
                }
            };
            // bitcoincore-rpc 0.19 cannot decode `getblockchaininfo` on the
            // CI bitcoind (`softforks` is absent). That comparison is a
            // sanity check, not a gate: skip it and keep waiting for funds.
            match rpc.client.get_blockchain_info() {
                ...
                Err(err) => {
                    log::debug!("skipping bitcoind tip check: {err}");
                }
            }
            match wallet.get_onchain_balance().await {
                ...
                Ok(_) => Ok(()),
                Err(err) => {
                    log::warn!("balance read failed while waiting for coins: {err}");
                    Err(())
                }
            }
        });
