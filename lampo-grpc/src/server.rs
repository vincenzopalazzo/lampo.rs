...
impl Lightning for GrpcServer {
    async fn get_info(
        ...
        let lampo_info: lampo_resp::GetInfo = serde_json::from_value(result).map_err(|e| {
            Status::internal(format!("Failed to deserialize daemon response: {}", e))
        })?;
    ...
    async fn connect_peer(
        ...
        let lnd_addr = req
            .addr
            .ok_or_else(|| Status::invalid_argument("LightningAddress is required"))?;

        let (host, port_str) = lnd_addr
            .host
            .rsplit_once(':')
            .ok_or_else(|| Status::invalid_argument("Address must contain host:port"))?;
        let port = port_str
            .parse::<u64>()
    ...
    async fn new_address(
        ...
        Err(Status::unimplemented("new_address"))
    }
    ...
    async fn list_channels(
        &self,
        _: Request<ListChannelsRequest>,
    ) -> Result<Response<ListChannelsResponse>, Status> {
        Err(Status::unimplemented("list_channels"))
    }
    async fn closed_channels(
        &self,
        _: Request<ClosedChannelsRequest>,
    ...
    async fn send_payment_sync(
        ...
        Err(Status::unimplemented("send_payment_sync"))
    }
    async fn send_to_route_sync(
        &self,
        _: Request<SendToRouteRequest>,
    ) -> Result<Response<SendResponse>, Status> {
    ...
    async fn add_invoice(
        ...
        Err(Status::unimplemented("add_invoice"))
    }
    async fn list_invoices(
        &self,
        _: Request<ListInvoiceRequest>,
    ) -> Result<Response<ListInvoiceResponse>, Status> {
        Err(Status::unimplemented("list_invoices"))
    }
    async fn lookup_invoice(&self, _: Request<PaymentHash>) -> Result<Response<Invoice>, Status> {
        Err(Status::unimplemented("lookup_invoice"))
    }
    async fn decode_pay_req(&self, _: Request<PayReqString>) -> Result<Response<PayReq>, Status> {
        Err(Status::unimplemented("decode_pay_req"))
    }
    ...
    type SubscribePeerEventsStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<PeerEvent, Status>> + Send>>;
    type SubscribeChannelEventsStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<ChannelEventUpdate, Status>> + Send>>;
    type OpenChannelStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<OpenStatusUpdate, Status>> + Send>>;
    type CloseChannelStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<CloseStatusUpdate, Status>> + Send>>;
    type SendPaymentStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<SendResponse, Status>> + Send>>;
    type SendToRouteStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<SendResponse, Status>> + Send>>;
    type SubscribeInvoicesStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<Invoice, Status>> + Send>>;
    type SubscribeChannelGraphStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<GraphTopologyUpdate, Status>> + Send>>;
    type SubscribeChannelBackupsStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<ChanBackupSnapshot, Status>> + Send>>;
    type SubscribeCustomMessagesStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<CustomMessage, Status>> + Send>>;
    type ChannelAcceptorStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<ChannelAcceptRequest, Status>> + Send>>;
    type RegisterRPCMiddlewareStream =
        std::pin::Pin<Box<dyn futures::Stream<Item = Result<RpcMiddlewareRequest, Status>> + Send>>;
    ...
    async fn subscribe_peer_events(
        &self,
        _: Request<PeerEventSubscription>,
    ...
    async fn subscribe_channel_events(
        &self,
        _: Request<ChannelEventSubscription>,
    ...
    async fn subscribe_channel_graph(
        &self,
        _: Request<GraphTopologySubscription>,
    ...
    async fn subscribe_channel_backups(
        &self,
        _: Request<ChannelBackupSubscription>,
    ...
    async fn subscribe_custom_messages(
        &self,
        _: Request<SubscribeCustomMessagesRequest>,
    ...
    async fn channel_acceptor(
        &self,
        _: Request<Streaming<ChannelAcceptResponse>>,
    ...
    async fn register_rpc_middleware(
        &self,
        _: Request<Streaming<RpcMiddlewareResponse>>,
