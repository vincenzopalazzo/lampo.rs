...
pub type HookHandler =
    Arc<dyn Fn(Value) -> Pin<Box<dyn Future<Output = HookResponse> + Send>> + Send + Sync>;
...
/// Runs during `init`, before the plugin acks. Return Err to disable.
...
pub struct Plugin {
    ...
    /// `init` params from the daemon, set before RPC methods run.
...
impl Plugin {
    ...
    /// `init` params the daemon sent. Empty until the handshake finishes.
    pub fn init(&self) -> Arc<RwLock<Option<Value>>> {
        self.init.clone()
    }

    /// Handle `init` before the ack. The daemon will not send RPC until this returns.
    pub fn on_init<F, Fut>(mut self, handler: F) -> Self
    where
        F: Fn(Value) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = Result<(), String>> + Send + 'static,
    {
        self.on_init = Some(Arc::new(move |params| {
            Box::pin(handler(params)) as Pin<Box<dyn Future<Output = Result<(), String>> + Send>>
        }));
        self
    }

    /// Register an RPC method with a handler.
    pub fn rpc_method<F, Fut>(
        mut self,
        name: &str,
        description: &str,
        usage: &str,
    ...
    /// Daemon refuses to finish startup if this plugin fails to start.
    pub fn important(mut self, important: bool) -> Self {
        self.manifest.important = important;
        self
    }
