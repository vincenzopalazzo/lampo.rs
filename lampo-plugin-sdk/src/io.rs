...
pub async fn run_plugin<R, W>(
    ...
    init: Option<Arc<RwLock<Option<serde_json::Value>>>>,
    on_init: Option<crate::InitHandler>,
) where
    R: AsyncRead + Unpin,
    W: AsyncWrite + Unpin,
{
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        let msg: Value = match serde_json::from_str(&line) {
            Ok(v) => v,
            Err(e) => {
                log::warn!(target: "plugin-sdk", "invalid JSON: {e}");
                continue;
            }
        };
        ...
        let response = match method.as_str() {
            "getmanifest" => {
                let manifest_json =
                    serde_json::to_value(&manifest).unwrap_or(Value::Object(Default::default()));
                serde_json::json!({
                    "jsonrpc": "2.0",
                    "id": id,
            ...
            "init" => {
                if let Some(slot) = &init {
                    *slot.write().await = Some(params.clone());
                }
            ...
            m if m.starts_with("hook/") => {
                ...
                if let Some(handler) = hook_handlers.get(hook_name) {
                    ...
                    let result = serde_json::to_value(&hook_resp)
        ...
        if let Err(e) = output.write_all(resp_str.as_bytes()).await {
            log::error!(target: "plugin-sdk", "failed to write response: {e}");
            return;
        }
        if let Err(e) = output.flush().await {
            log::error!(target: "plugin-sdk", "failed to flush output: {e}");
            return;
        }
    }
}
