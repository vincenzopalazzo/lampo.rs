...
use lampo_common::handler::EventForSpecificLDKVersion;
...
pub struct VanillaLDKVersion {
    channel_manager: Arc<LampoChannelManager>,
}

impl EventForSpecificLDKVersion for VanillaLDKVersion { 
    fn handler(&self, handler: &dyn EventHandler, event: ldk::events::Event) -> error::Result<()> {
        match event {
            ldk::events::Event::ChannelClosed {
                ...
                handler.emit(Event::Lightning(LightningEvent::CloseChannelEvent { channel_id: channel_id.to_string(), message: reason.to_string(), counterparty_node_id : node_id, funding_utxo : txo}));
...
pub struct LampoHandler {
    ...
    event_handler_for_specific_ldk_version: Arc<dyn EventForSpecificLDKVersion>,
    #[allow(dead_code)]
...
impl LampoHandler {
    pub(crate) fn new(lampod: &LampoDaemon) -> Self {
        ...
        Self {
            ...
            // FIXME: be able to mutate with a RefCell.
            event_handler_for_specific_ldk_version: Arc::new(VanillaLDKVersion {
                channel_manager: lampod.channel_manager(),
            }),
            emitter,
            subscriber,
    ...
    // FIXME: this should be mutable :) use RefCell
    pub fn set_handler_specific_ldk_version(&self, handler: Arc<dyn EventForSpecificLDKVersion>) {
        unimplemented!()
    }
    ...
    fn handle(&self, event: ldk::events::Event) -> error::Result<()> {
        match event {
            ...
            ldk::events::Event::ChannelClosed {
               ...
               self.event_handler_for_specific_ldk_version.handler(self, event) 
            }
            ...
            ldk::events::Event::PaymentClaimable { .. } => self.event_handler_for_specific_ldk_version.handler(self, event),
            ldk::events::Event::PaymentClaimed {
                receiver_node_id,
                payment_hash,
                amount_msat,
